...
use bevy_ecs_ldtk::{ldtk::Level, prelude::LdtkFields};
...
pub fn get_level_music(level: &Level) -> Option<String> {
    level.get_field("level_music").ok().and_then(|field| {
        match field {
            bevy_ecs_ldtk::ldtk::FieldValue::String(s) => s.clone(),
            _ => None,
        }
    })
}
